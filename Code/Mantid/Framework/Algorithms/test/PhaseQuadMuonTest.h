...
public:
  void testName() { TS_ASSERT_EQUALS(phaseQuadMuon.name(), "PhaseQuad"); }

  void testCategory() { TS_ASSERT_EQUALS(phaseQuadMuon.category(), "Muon") }
  ...
  void testExecPhaseList() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        phaseQuadMuon.setProperty("PhaseList", "TestPhaseList.txt"));
    TS_ASSERT_THROWS_NOTHING(
        phaseQuadMuon.setProperty("InputWorkspace", "EMU6473"));
    TS_ASSERT_THROWS_NOTHING(
        phaseQuadMuon.setProperty("OutputWorkspace", "EMU6473_out"));
  ...
  void generatePhaseList(std::string filename) {
    ...
    if (ofile.is_open()) {
      ...
      ofile << "#histos, typ. first good bin#, typ. bin# when pulse over, mean "
               "lag." << std::endl;
      ofile << "Tabulated numbers are, per histogram:" << std::endl;
      ofile << "det ok, asymmetry, phase, lag, deadtime_c, deadtime_m."
            << std::endl;
  ...
  void testExecPhaseTable() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        phaseQuadMuon.setProperty("OutputWorkspace", "EMU6473_out"));
    TS_ASSERT_THROWS_NOTHING(
        phaseQuadMuon.setProperty("PhaseTable", "PhaseTable"));
