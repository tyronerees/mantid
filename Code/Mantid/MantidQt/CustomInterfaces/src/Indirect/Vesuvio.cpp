...
Vesuvio::Vesuvio(QWidget *parent):
...
void Vesuvio::initLayout()
{
  ...
  m_tabs.insert(std::make_pair(LOAD, new VesuvioLoad(m_uiForm.twVesuvioTabs->widget(LOAD))));
  m_tabs.insert(std::make_pair(CORRECTIONS, new VesuvioCorrections(m_uiForm.twVesuvioTabs->widget(CORRECTIONS))));
  m_tabs.insert(std::make_pair(FITTING, new VesuvioFitting(m_uiForm.twVesuvioTabs->widget(FITTING))));
...
{
  MantidQt::API::HelpWindow::showCustomInterface(NULL, QString("Indirect_VESUVIO"));
}
